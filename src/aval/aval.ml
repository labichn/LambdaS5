...
module AValueSet = Set.Make (AValue)
module ObjectV = struct
  type t = V.objectv
  let compare = Pervasives.compare
end
module ObjectVSet = Set.Make (ObjectV)
type astore = (ObjectVSet.t ST.t * AValueSet.t ST.t)
let empty = ((LocMap.empty, 0), (LocMap.empty, 0))
type addr = ST.loc
type state =
| Ev of S.exp * V.env * astore * K.akont
| Co of K.akont * avalue * astore
| Ap of avalue * avalue * astore * K.akont
| Ans of V.value * astore
module State = struct
  type t = state
  let compare = Pervasives.compare
end
...
let try_get_vars (_, vars) l =
  if AValueSet.mem l vars
  match try V.get_maybe_var s l with Not_found -> AValueSet.empty with
  | Some vals -> vals
  | None -> AValueSet.empty

(* aval : (string -> Ljs_syntax.exp) state int StateSet => StateSet *)
let rec aval desugar state i ss =
  begin
    let aval' s =
      if StateSet.mem s ss then ss
      else aval desugar s (i+1) (StateSet.add s ss) in
    match state with
    | Ans (_, _) -> ss
    | Co (K.Mt, valu, store) ->
      (match valu with
      | Val valu' -> aval' (Ans (valu', store))
      | _ -> failwith "got a kont as final value... weird")
    (* value cases *)
    | Ev (S.Undefined _, _, s, k) -> aval' (Co (k, Val V.Undefined, s))
    | Ev (S.Null _, _, s, k) -> aval' (Co (k, Val V.Null, s))
    | Ev (S.String (_, str), _, s, k) -> aval' (Co (k, Val (V.String str), s))
    | Ev (S.Num (_, n), _, s, k) -> aval' (Co (k, Val (V.Num n), s))
    | Ev (S.True _, _, s, k) -> aval' (Co (k, Val V.True, s))
    | Ev (S.False _, _, s, k) -> aval' (Co (k, Val V.False, s))
    | Ev (S.Id (p, x), e, s, k) ->
      let vals = try_get_vars s (P.IdMap.find x e) in
      if AValueSet.cardinal vals == 0 then
        failwith ("[interp] Unbound identifier: " ^ name ^
                  " in identifier lookup at " ^ (P.Pos.string_of_pos p))
      else
        AValueSet.fold
          (fun vall ss' -> match vall with
          | Val _ -> StateSet.union (aval' (Co (k, vall, s))) ss'
          | _ -> ss')
          ss
          vals
...
let continue_eval expr desugar print_trace env store =
  try
    ...
    let ss = aval desugar (Ev (expr env store K.Mt)) StateSet.empty 0 in
    begin
      print_endline "reached fixpoint";
      print_endline ("set size: "^(string_of_int (StateSet.cardinal ss)));
    end
    L.Answer ([], V.True, [], store) (* pass back true to show we're done *)
  with
  | _ -> L.Answer ([], V.False, [], store)

let eval_expr expr desugar print_trace =
  continue_eval expr desugar print_trace P.IdMap.empty empty
