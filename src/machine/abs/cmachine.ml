...
(*        print_endline "entering ap state";
    ...
    | C.EvA (p, attrs, env, h, k, t) ->
      ...
      | [] ->
        ...
        eval' (C.coa attrsv h k)
      | (name, exp)::pairs ->
        eval_k' (C.ev exp env h) (K.Attrs (p, t, name, pairs, [], kls, ext, env, k)))
    | C.Ev (SYN.GetAttr (p, attr, obj, field), env, h, k, t) ->
      eval_k' (C.ev obj env h) (K.GetAttr (p, t, attr, field, env, k))
    | C.Ev (SYN.SetAttr (p, pattr, obj, field, next), env, h, k, t) ->
      eval_k' (C.ev obj env h) (K.SetAttr (p, t, pattr, field, next, env, k))
    | C.Ev (SYN.GetObjAttr (p, oattr, obj), env, h, k, t) ->
      eval_k' (C.ev obj env h) (K.GetObjAttr (p, t, oattr, env, k))
    | C.Ev (SYN.SetObjAttr (p, oattr, obj, next), env, h, k, t) ->
      eval_k' (C.ev obj env h) (K.SetObjAttr (p, t, oattr, next, env, k))
    | C.Ev (SYN.GetField (p, obj, field, body), env, h, k, t) ->
      eval_k' (C.ev obj env h) (K.GetField (p, t, field, body, env, k))
    | C.Ev (SYN.OwnFieldNames (p, obj), env, h, k, t) ->
      eval_k' (C.ev obj env h) (K.OwnFieldNames (p, t, k))
    | C.Ev (SYN.DeleteField (p, obj, field), env, h, k, t) ->
      eval_k' (C.ev obj env h) (K.DeleteField (p, t, field, env, k))
    | C.Ev (SYN.SetField (p, obj, field, next, body), env, h, k, t) ->
      eval_k' (C.ev obj env h) (K.SetField (p, t, field, next, body, env, k))
    | C.Ev (SYN.Op1 (p, name, arg), env, h, k, t) ->
      eval_k' (C.ev arg env h) (K.OpOne (p, t, name, env, k))
    | C.Ev (SYN.Op2 (p, name, arg1, arg2), env, h, k, t) ->
      eval_k' (C.ev arg1 env h) (K.OpTwo (p, t, name, arg2, env, k))
    | C.Ev (SYN.If (p, pred, than, elze), env, h, k, t) ->
      eval_k' (C.ev pred env h) (K.If (p, t, env, than, elze, k))
    | C.Ev (SYN.App (p, func, args), env, h, k, t) ->
      eval_k' (C.ev func env h) (K.App (p, t, env, args, k))
    | C.Ev (SYN.Seq (p, left, right), env, h, k, t) ->
      eval_k' (C.ev left env h) (K.Seq (p, t, right, env, k))
    | C.Ev (SYN.Let (p, name, expr, body), env, h, k, t) ->
      eval_k' (C.ev expr env h) (K.Let (p, t, name, body, env, k))
    ...
    | C.Ev (SYN.Label (p, name, exp), env, h, k, t) ->
      eval_h' (C.ev exp env) (H.Lab (p, t, name, env, k, h))
    | C.Ev (SYN.Break (p, label, expr), env, h, k, t) ->
      eval_k' (C.ev expr env h) (K.Break (p, t, label, env, k))
    | C.Ev (SYN.TryCatch (p, body, catch), env, h, k, t) ->
      eval_h' (C.ev body env) (H.Cat (p, t, catch, env, k, h))
    | C.Ev (SYN.TryFinally (p, body, fin), env, h, k, t) ->
      eval_h' (C.ev body env) (H.Fin (p, t, fin, env, k, h))
    | C.Ev (SYN.Throw (p, expr), env, h, k, t) ->
      eval_k' (C.ev expr env h) (K.Throw (p, t, env, k))
    | C.Ev (SYN.Eval (p, str, bindings), env, h, k, t) ->
      eval_k' (C.ev str env h) (K.Eval (p, t, bindings, env, k))
    | C.Ev (SYN.Hint (p, "___takeS5Snapshot", expr), env, h, k, t) ->
      eval_k' (C.ev expr env h) (K.Hint (p, t, env, k))
    | C.Ev (SYN.Hint (p, _, expr), env, h, k, t) ->
      eval' (C.ev expr env h k)
    | C.Ex _ -> (* exception cases match on handle *)
      (match ST.get_hand (C.hand_of state) store, state with
      | H.Lab (p, t', name, _, k, h), C.Ex ((E.Break (_, label, v) as brk), env, _, t) ->
        if name = label then eval' (C.co v h k)
        else eval' (C.ex brk env h)
      | H.Cat (p, _, catch, env, k, h), C.Ex (E.Throw (_, throw_val), _, _, t) ->
        eval_k' (C.ev catch env h) (K.Catch (p, t, throw_val, env, k))
      | H.Fin (p, _, fin, env, k, h), C.Ex (ex, _, _, t) ->
        eval_k' (C.ev fin env h) (K.Finally (p, t, ex, env, k))
      | H.MtH _, C.Ex (ex, env, _, t) -> raise ex
      | h, C.Ex ((E.Break (_, _, _) as brk), env, _, t) ->
        eval' (C.ex brk env (H.hand_of h))
      | h, C.Ex ((E.Throw _ as thr), env, _, t) ->
        eval' (C.ex thr env (H.hand_of h))
      | h, C.Ex (ex, env, _, t) ->
        eval' (C.ex ex env (H.hand_of h))
      | _ -> failwith "Encountered an unmatched machine state.")
    | _ ->
      let kont = ST.get_kont (C.kont_of state) store in
      (match state, kont with
      | C.Co (_, v, h, _), K.SetBang (p, t, loc, k) ->
        let store' = ST.set_val loc v store in
        eval'' store' (C.co v h k)
      | C.CoA (_, valu, h, _), K.Object (p, t, [], _, k) -> (* empty props case *)
        let obj_loc = CT.alloc state store (Some kont) None in
        let store' = ST.add_obj obj_loc (valu, P.IdMap.empty) store in
        eval'' store' (C.co (V.Obj obj_loc) h k)
      | C.CoA (_, attrsv, h, _), K.Object (p, t, prop::ps, env, k) ->
        eval_k' (C.evp p prop env h) (K.Object2 (p, t, attrsv, ps, [], env, k))
      | C.CoP (_, pv, h, _), K.Object2 (p, t, attrsv, prop::ps, pvs, env, k) ->
        eval_k' (C.evp p prop env h) (K.Object2 (p, t, attrsv, ps, pv::pvs, env, k))
      | C.CoP (_, pv, h, _), K.Object2 (p, t, attrsv, [], pvs, env, k) ->
        let add_prop acc (name, propv) = P.IdMap.add name propv acc in
        let propsv = List.fold_left add_prop P.IdMap.empty (pv::pvs) in
        let obj_loc = ST.alloc context store (Some kont) None in
        let store' = ST.add_obj obj_loc (attrsv, propsv) store in
        eval'' store' (C.co (V.Obj obj_loc) h k)
      | C.Co (_, valu, h, _), K.DataProp (p, t, name, w, enum, config, k) ->
        eval' (C.cop (name, V.Data ({ V.value=valu; V.writable=w; }, enum, config)) h k)
      | C.Co (_, getter_val, h, _), K.AccProp (p, t, name, setter, enum, config, env, k) ->
        eval_k' (C.ev setter env h)
          (K.AccProp2 (p, t, name, getter_val, enum, config, env, k))
      | C.Co (_, sv, h, _), K.AccProp2 (p, t, name, gv, enum, config, env, k) ->
        eval' (C.cop (name, V.Accessor ({ V.getter=gv; V.setter=sv; }, enum, config)) h k)
      | C.Co (_, valu, h, _), K.Attrs (p, t, name, (name', exp)::pairs, valus, kls, ext, env, k) ->
        eval_k' (C.ev exp env h)
          (K.Attrs (p, t, name', pairs, (name, valu)::valus, kls, ext, env, k))
      | C.Co (_, valu, h, _), K.Attrs (p, t, name, [], valus, kls, ext, env, k) ->
        ...
        let attrsv = { V.code=(opt_get "code" valus);
                       ...
                       V.extensible=ext; } in
        eval' (C.coa attrsv h k)
      | C.Co (_, obj_val, h, _), K.GetAttr (p, t, attr, field, env, k) ->
        eval_k' (C.ev field env h) (K.GetAttr2 (p, t, attr, obj_val, env, k))
      | C.Co (_, field_val, h, _), K.GetAttr2 (p, t, attr, obj_val, env, k) ->
        eval' (C.co (Cdelta.get_attr attr obj_val field_val store) h k)
      | C.Co (_, obj_val, h, _), K.SetAttr (p, t, pattr, field, next, env, k) ->
        eval_k' (C.ev field env h) (K.SetAttr2 (p, t, pattr, next, obj_val, env, k))
      | C.Co (_, field_val, h, _), K.SetAttr2 (p, t, pattr, next, obj_val, env, k) ->
        eval_k' (C.ev next env h) (K.SetAttr3 (p, t, pattr, obj_val, field_val, env, k))
      | C.Co (_, valu, h, _), K.SetAttr3 (p, t, pattr, obj_val, field_val, env, k) ->
        let b, store' = D.set_attr pattr obj_val field_val valu store in
        eval'' store' (C.co (V.Bool b) h k)
      | C.Co (_, obj_val, h, _), K.GetObjAttr (p, t, oattr, env, k) ->
        (match obj_val with
        | V.Obj obj_loc -> (match ST.get_obj obj_loc store with
          | (attrs, _) -> eval' (C.co (D.get_obj_attr attrs oattr) h k)
          | _ -> failwith "[interp] GetObjAttr got a non-object."))
        | C.Co (_, obj_val, h, _), K.SetObjAttr (p, t, oattr, next, env, k) ->
          eval_k' (C.ev next env h) (K.SetObjAttr2 (p, t, oattr, obj_val, env, k))
        | C.Co (_, valu, h, _), K.SetObjAttr2 (p, t, oattr, obj_val, env, k) ->
          (match obj_val with
          | V.Obj loc -> (match ST.get_obj loc store with
            | (attrs, props) ->
              ...
              eval'' (ST.set_obj loc (attrs', props) store) (C.co valu h k))
          | _ -> failwith "[interp] SetObjAttr got a non-object")
      | C.Co (_, obj_val, h, _), K.GetField (p, t, field, body, env, k) ->
        eval_k' (C.ev field env h) (K.GetField2 (p, t, body, obj_val, env, k))
      | C.Co (_, field_val, h, _), K.GetField2 (p, t, body, obj_val, env, k) ->
        eval_k' (C.ev body env h) (K.GetField3 (p, t, obj_val, field_val, env, k))
      | C.Co (_, body_val, h, _), K.GetField3 (p, t, obj_val, field_val, env, k) ->
        (match (obj_val, field_val) with
        | (V.Obj _, V.String s) -> (match D.get_prop p obj_val s store with
          | Some (V.Data ({V.value=v;}, _, _)) -> eval' (C.co v h k)
          | Some (V.Accessor ({V.getter=g;},_,_)) ->
            eval' (C.ap p g [obj_val; body_val] h k)
          | None -> eval' (C.co V.Undef h k))
        | _ -> failwith ("[interp] Get field didn't get an object and a string at "
                         ^ P.Pos.string_of_pos p ^ "."))
      | C.Co (_, obj_val, h, _), K.OwnFieldNames (p, t, k) ->
        (match obj_val with
        | V.Obj loc -> (match ST.get_obj loc store with
          | (_, props) ->
            let add_name n x m =
      ...
      | C.Co (_, obj_val, h, _), K.DeleteField (p, t, field, env, k) ->
        eval_k' (C.ev field env h) (K.DeleteField2 (p, t, obj_val, env, k))
      | C.Co (_, field_val, h, _), K.DeleteField2 (p, t, obj_val, env, k) ->
        (match obj_val, field_val with
        | V.Obj loc, V.String s ->
          ...
          | (attrs, props) ->
            ...
            | V.Accessor (_, _, true) ->
              ...
              eval'' store' (C.co (V.Bool true) h k)
            | _ -> eval' (C.ex (E.Throw ([], V.String "unconfigurable-delete")) env h)
             with Not_found -> eval' (C.co (V.Bool false) h k)))
        | _ -> failwith ("[interp] Delete field didn't get an object and a string at "
                         ^ P.Pos.string_of_pos p))
      | C.Co (_, obj_val, h, _), K.SetField (p, t, field, next, body, env, k) ->
        eval_k' (C.ev field env h) (K.SetField2 (p, t, next, body, obj_val, env, k))
      | C.Co (_, field_val, h, _), K.SetField2 (p, t, next, body, obj_val, env, k) ->
        eval_k' (C.ev next env h) (K.SetField3 (p, t, body, obj_val, field_val, env, k))
      | C.Co (_, valu, h, _), K.SetField3 (p, t, body, obj_val, field_val, env, k) ->
        eval_k' (C.ev body env h) (K.SetField4 (p, t, obj_val, field_val, valu, env, k))
      | C.Co (_, body_val, h, _), K.SetField4 (p, t, obj_val, field_val, valu, env, k) ->
        (match (obj_val, field_val) with
        | (V.Obj loc, V.String s) -> (match ST.get_obj loc store with
          | ({V.extensible=extensible;} as attrs, props) ->
            ...
            | Some (V.Data ({ V.writable = true; }, enum, config)) ->
              ...
              eval'' store' (C.co valu h k)
            | Some (V.Data _) ->
              eval' (C.ex unwritable env h)
            | Some (V.Accessor ({ V.setter = V.Undef; }, _, _)) ->
              eval' (C.ex unwritable env h)
            | Some (V.Accessor ({ V.setter = setterv; }, _, _)) ->
              eval' (C.ap p setterv [obj_val; body_val] h k)
            | None ->
              if extensible
              then
                ...
                let store' = ST.set_obj loc (attrs, props) store in
                eval'' store' (C.co valu h k)
              else eval' (C.co V.Undef h k)))
        | _ -> failwith ("[interp] Update field didn't get an object and a string"
                         ^ P.Pos.string_of_pos p))
      | C.Co (_, arg_val, h, _), K.OpOne (p, t, name, env, k) ->
        eval' (C.co (D.op1 store name arg_val) h k)
      | C.Co (_, arg1_val, h, _), K.OpTwo (p, t, name, arg2, env, k) ->
        eval_k' (C.ev arg2 env h) (K.OpTwo2 (p, t, name, arg1_val, env, k))
      | C.Co (_, arg2_val, h, _), K.OpTwo2 (p, t, name, arg1_val, env, k) ->
        eval' (C.co (D.op2 store name arg1_val arg2_val) h k)
      | C.Co (_, pred_val, h, _), K.If (p, t, env, than, elze, k) ->
        (match pred_val with
        | V.Bool true -> eval' (C.ev than env h k)
        | _ -> eval' (C.ev elze env h k))
      | C.Co (_, func, h, _), K.App (p, t, env, [], k) -> (* no arg apps *)
        eval' (C.ap p func [] h k)
      | C.Co (_, func, h, _), K.App (p, t, env, expr::exprs, k) ->
        eval_k' (C.ev expr env h) (K.App2 (p, t, func, env, [] , exprs, k))
      | C.Co (_, arg_val, h, _), K.App2 (p, t, func, env, vs, expr::exprs, k) ->
        eval_k' (C.ev expr env h) (K.App2 (p, t, func, env, arg_val::vs, exprs, k))
      | C.Co (_, arg_val, h, _), K.App2 (p, t, func, env, vs, [], k) ->
        eval' (C.ap p func (List.rev (arg_val::vs)) h k)
      | C.Co (_, _, h, _), K.Seq (p, t, right, env, k) ->
        eval' (C.ev right env h k)
      | C.Co (_, v, h, _), K.Let (p, t, name, body, env, k) ->
        let new_loc = ST.alloc context store (Some kont) None in
        let store' = ST.add_val new_loc v store in
        eval'' store' (C.ev body (P.IdMap.add name new_loc env) h k)
      | C.Co (_, v, h, _), K.Rec (p, t, new_loc, body, env, k) ->
        eval'' (ST.set_val new_loc v store) (C.ev body env h k)
      | C.Co (_, v, h, _), K.Break (p, t, label, env, k) ->
        eval' (C.ex (E.Break ([], label, v)) env h)
      | C.Co (_, catch_val, h, _), K.Catch (p, t, throw_val, env, k) ->
        eval' (C.ap p catch_val [throw_val] h k)
      | C.Co (_, _, h, _), K.Finally (p, t, ex, env, k) ->
        (match ex with
        | E.Throw (th, v) ->
          eval' (C.ex (E.Throw (th, v)) env h)
        | E.Break (th, l, v) ->
          eval' (C.ex (E.Break (th, l, v)) env h)
        | _ -> failwith "try finally caught something other than a throw or break.")
      | C.Co (_, _, h, _), K.Finally2 (p, t, valu, k) ->
        eval' (C.co valu h k)
      | C.Co (_, valu, h, _), K.Throw (p, t, env, k) ->
        eval' (C.ex (E.Throw ([], valu)) env h)
      | C.Co (_, str_val, h, _), K.Eval (p, t, bindings, env, k) ->
        eval_k' (C.ev bindings env h) (K.Eval2 (p, t, str_val, env, k))
      | C.Co (_, bind_val, h, _), K.Eval2 (p, t, str_val, env, k) ->
        (match str_val, bind_val with
        | V.String s, V.Obj o ->
          let expr = desugar s in
          let new_locs = ST.alloc' context store (Some kont) None in
          let env', store' = ST.envstore_of_obj new_locs (ST.get_obj o store) store in
          eval desugar context store' (C.ev expr env' h k)
        | V.String _, _ -> E.interp_error p "Non-object given to eval() for env"
        | v, _ -> eval' (C.co v h k))
      | C.Co (_, valu, h, _), K.Hint (p, t, env, k) ->
        eval' (C.ex (E.Snapshot (valu, env, store)) env h)
      | _, _ -> (match context, kont, ST.get_hand (C.hand_of context) store with
        | C.Co (_, valu, _, _), K.Mt _, H.MtH _ -> eval' (C.ans valu) (* victory! *)
        | C.Co (_, valu, _, _), K.Mt _, H.Lab (p, t, name, env, k, h) ->
          eval' (C.co valu h k)
        | C.Co (_, valu, _, _), K.Mt _, H.Cat (p, t, _, _, k, h) ->
          eval' (C.co valu h k)
        | C.Co (_, valu, _, _), K.Mt _, H.Fin (p, t, exp, env, k, h) ->
          eval_k' (C.ev exp env h) (K.Finally2 (p, t, valu, k))
        | _ -> begin
...
let inj exp env store =
  ...
  C.Ev (exp, env, t, t, t+1), store'

let continue_eval exp desugar env store : A.answer =
  try
    Sys.catch_break true;
    let context, store' = inj exp env store in
    machine_eval desugar context store'
  with
